    ...
    <div className="mb-16 last:mb-0">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <span className="inline-flex items-center gap-1.5 bg-gold/10 border border-gold/20 rounded-full px-3 py-1.5">
            <Sparkles className="w-3 h-3 text-gold" />
            <span className="text-gold text-[10px] font-semibold tracking-widest uppercase">
              {badge}
            </span>
          </span>
          <h3 className="font-[var(--font-playfair)] text-2xl md:text-3xl font-bold text-cream">
            {title}
          </h3>
        </div>
        <Link
          href={href}
          className="text-gold text-sm font-medium hover:text-gold-light transition-colors hidden sm:inline-flex items-center gap-1"
        >
          View All
      ...
      <div className="flex justify-center mt-6 sm:hidden">
        <Link
          href={href}
          className="text-gold text-sm font-medium hover:text-gold-light transition-colors inline-flex items-center gap-1"
        >
          View All {title}
          <ChevronRight className="w-4 h-4" />
        </Link>
...
export default function Collection() {
  const [reedProducts, setReedProducts] = useState<Product[]>([]);
  const [carProducts, setCarProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([
      fetch("/api/products?category=reed-diffusers", { cache: "no-store" }).then((r) => r.json()),
      fetch("/api/products?category=car-diffusers", { cache: "no-store" }).then((r) => r.json()),
    ])
      .then(([reed, car]) => {
        setReedProducts(Array.isArray(reed) ? reed : []);
        setCarProducts(Array.isArray(car) ? car : []);
        setLoading(false);
      })
      .catch(() => {
        setReedProducts([]);
  ...
  if (loading) {
    return (
      <section
        id="collection"
        className="py-24 md:py-32 bg-forest relative min-h-[500px] flex justify-center items-center"
      >
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-16 md:mb-20">
          ...
          <p className="text-cream/70 mt-6 max-w-2xl mx-auto text-lg font-light leading-relaxed">
            Premium fragrances in luxurious glass bottles — REED diffusers for
            your space, car diffusers for the road.
          </p>
        </div>

        {/* REED Diffusers Carousel */}
        <CategoryCarousel
